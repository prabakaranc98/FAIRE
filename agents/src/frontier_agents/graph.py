...
Two graphs:
1. `build_wiki_graph()` — full editorial pipeline (research → write → review → commit)
2. `build_mvb_graph()`  — MVB-only pipeline (HF search → generate MVB → merge → review → commit)
"""
...
def build_wiki_graph() -> StateGraph:
    """Full editorial pipeline: persona → stub → research → draft → review → commit.
    ...
    Graph topology:
        START
          ...
          → research
          → write_draft
          → review ─────────────── [approved] ──→ write_file → commit → END
                    └── [revise, count<2] ──→ revise_draft → review
                    └── [revise, count≥2] ──→ flag_human_review → END
    """
    ...
    graph.add_edge("flag_human_review", END)

    return graph


def build_mvb_graph() -> StateGraph:
    """MVB-only pipeline: persona → stub → MVB search → generate → merge → review → commit.

    Graph topology:
        START
          → load_persona
          → read_stub
          → mvb_recipe  (HF model/dataset search + Claude generation)
    ...
    graph.add_edge(START, "load_persona")
    graph.add_edge("load_persona", "read_stub")
    graph.add_edge("read_stub", "mvb_recipe")
