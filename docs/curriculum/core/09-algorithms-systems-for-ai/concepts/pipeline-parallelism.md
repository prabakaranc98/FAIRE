...
Where this concept appears: this page is foundational for the arc steps that look at combining parallelism types, namely *pipelines* <!-- [[pipelines]] -->, and it feeds directly into practical deployment guides such as [Model Parallelism](model-parallelism.md). When you reach those steps, you can point back to this scheduling view to understand why the choice of micro-batch depth matters as much as the layer split itself.
...
If you want the systems-level groundwork for overlapping work and communication, → [Data Parallelism](data-parallelism.md) explains how gradients aggregate across replicas and why pipeline bubbles break that aggregation. If you want the kernel-level levers that shrink \(\tau_f\) and \(\tau_b\), → [Flash Attention](flash-attention.md) describes the tiling tricks and fused kernels that keep stage runtime predictable. To explore how computation becomes a continuous flow that suggests more fluid partition boundaries, → [Flow Matching](../../02-generative-modeling/concepts/flow-matching.md) draws the theoretical analogy that lets you see pipeline stages as control points on a flow.
