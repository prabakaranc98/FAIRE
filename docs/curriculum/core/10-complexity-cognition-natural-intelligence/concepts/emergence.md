...
If you want the algorithmic-complexity grounding, → [[concepts/circuit-complexity]] explains how function classes map to neural architectures; the engineering counterpart is → [[arcs/scaling-laws]] which shows how compute budgets trace the same contours; for hierarchical reasoning and synthetic phase transitions, → [[concepts/knowledge-tracing]] presents the benchmarks and data-generation practices that drop straight into the emergence simulations you just built.
