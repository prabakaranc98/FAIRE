...
If you want the probabilistic foundation, → [Score Matching](../../02-generative-modeling/concepts/score-matching.md) explains how backpropagation gradients approximate the data score function. The engineering counterpart is → *gradient checkpointing* <!-- [[gradient-checkpointing]] --> for reshaping cached activations when memory becomes the bottleneck, and the systems story continues in → [Flow Matching](../../02-generative-modeling/concepts/flow-matching.md) where continuous-time dynamics mimic the backward pass over noising trajectories. Connected topics include → *autodiff* <!-- [[autodiff]] --> for the computation-graph plumbing, → *optimizer design* <!-- [[optimizer-design]] --> for how gradients feed parameter updates, and → *hardware aware training* <!-- [[hardware-aware-training]] --> for the constraints that originally motivated the Lagrangian formulation.

## What can you build next

Scale the manual engine from this page into the curriculum’s heavier arcs: → *gradient checkpointing* <!-- [[gradient-checkpointing]] --> shows how to recompute caches selectively and trace the runtime/accuracy trade-off, → *optimizer design* <!-- [[optimizer-design]] --> matches the backward gradients to different optimizers, and → *hardware aware training* <!-- [[hardware-aware-training]] --> revisits the Lagrangian story with explicit energy or safety constraints so you learn how cached activations play in constrained deployments.
