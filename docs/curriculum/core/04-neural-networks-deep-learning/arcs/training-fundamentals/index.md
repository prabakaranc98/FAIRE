...
The conceptual counterparts are → [[curriculum/04-neural-networks-deep-learning/evaluation-metrics]] for instrumentation guidance and → [[curriculum/04-neural-networks-deep-learning/gradient-descent]] for the math that underpins AdamW’s moment estimators; connected topics include → [[curriculum/04-neural-networks-deep-learning/cnn-basics]] for the architectural backbones that consume these training loops and → [[curriculum/04-neural-networks-deep-learning/arcs/stabilizing-transformers]] for the arc step that applies this discipline to sequence models; this concept appears as the stabilizing node in the [[curriculum/04-neural-networks-deep-learning/arcs/training-fundamentals]] arc, which informs later stages such as [[curriculum/04-neural-networks-deep-learning/arcs/monitoring-training]], so succeeding arcs can reuse the same lab-book instruments.
...
**What you can build next:** Extend the instrumentation to Tiny ImageNet by following [[curriculum/04-neural-networks-deep-learning/arcs/scale-vision-training]] so the lab notebook becomes a template that compares optimizer variants every quarter and feeds into the larger arc’s telemetry dashboards.
